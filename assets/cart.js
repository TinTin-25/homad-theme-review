...
class CartItems extends HTMLElement {
  ...
  enableLoading(line) {
    ...
    const cartItemElements = this.querySelectorAll(`#CartItem-${line} .pre-loading`);
    const cartDrawerItemElements = this.querySelectorAll(`#CartDrawer-Item-${line} .pre-loading`);

    [...cartItemElements, ...cartDrawerItemElements].forEach((overlay) => overlay.classList.remove('d-none'));
  ...
  disableLoading(line) {
    ...
    [...cartItemElements, ...cartDrawerItemElements].forEach((overlay) => overlay.classList.add('d-none'));
  }
  ...
  enablePreOrder () {
    ...
    const editCartItemButton = document.querySelectorAll('.js-cartpage-edit');
    for (let j = 0; j < nodeList1.length; j++) {
      ...
      // Update button edit item on cart page
      editCartItemButton.forEach((button) => {
        var productHandle = button.dataset.handle;

        if (proOrderHandle == productHandle) {
          button.dataset.preOrder = true;
        }
      });
    }
  }
